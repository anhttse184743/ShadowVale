    ...
    public sealed class ForestGameplayMergeTests : ForestSceneTestBase
    {
        ...
        public IEnumerator MigratedPlayerAndEnemiesShareInventoryPauseAndCheckpoints()
        {
            ...
            try {
                ...
                mission.Damage(30);
                Assert.AreEqual(health.Current, mission.Health);
                combat.Equip(WeaponKind.Knife);
                int ammo = mission.Count("ammo_rifle");
                Assert.IsTrue(combat.TryConsumeRound());
                Assert.AreEqual(ammo - 1, mission.Count("ammo_rifle"));
                ...
                Assert.IsFalse(mission.CanSave(), "Alerted enemies must block manual backups.");
                Assert.IsTrue(mission.AutoSaveOnExit(out var error), error);
