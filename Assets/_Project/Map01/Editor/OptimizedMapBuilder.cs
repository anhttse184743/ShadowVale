...
namespace ShadowVale.Map01.Editor
{
    ...
    public static class OptimizedMapBuilder
    {
        ...
        static bool busy;
        static OptimizedMapBuilder() { EditorApplication.update += Poll; }
        static void Poll()
        {
        ...
        [Serializable] public class SourceData { public MeshData[] meshes; public TreeData[] trees; public BoxData[] boxes; public RouteData[] routes,patrols; public PointData[] points; public float[] spawn,hung,encounterExit; }
        ...
        public static void Build()
        {
            ...
            var data=Read();
            Directory.CreateDirectory(Root+"/Meshes");Directory.CreateDirectory(Root+"/Materials");Directory.CreateDirectory(Reports);AssetDatabase.Refresh();
            ...
            var opaque=MaterialAsset("MapPalette",shader,Color.white);
            ...
            try
            {
                ...
                {
                    var mesh=new Mesh{name=src.id,indexFormat=IndexFormat.UInt32};int n=src.vertices.Length/3;
                    ...
                    string path=Root+"/Meshes/"+src.id+".asset";var old=AssetDatabase.LoadAssetAtPath<Mesh>(path);
                    if(old==null) AssetDatabase.CreateAsset(mesh,path);else {EditorUtility.CopySerialized(mesh,old);Object.DestroyImmediate(mesh);mesh=old;EditorUtility.SetDirty(mesh);}
            ...
            {
                var go=Child("Tree "+index++,trees.transform);go.layer=blockLayer;go.transform.position=V(src.position);go.transform.localScale=V(src.scale);go.transform.rotation=Quaternion.Euler(0,src.yaw,0);
                ...
                var lods=new LOD[3];float[] heights={.26f,.10f,.008f};
                for(int lod=0;lod<3;lod++)
                {
                    var visual=Child("LOD"+lod,go.transform);visual.AddComponent<MeshFilter>().sharedMesh=meshAssets["Tree_"+src.type+"_LOD"+lod];
            ...
            mission.mapMin=new Vector2(-90,-80);mission.mapMax=new Vector2(90,80);
            ...
            var sun=new GameObject("Sun").AddComponent<Light>();sun.type=LightType.Directional;sun.intensity=1.6f;sun.color=new Color(1,.94f,.8f);sun.shadows=LightShadows.Soft;sun.transform.rotation=Quaternion.Euler(48,-35,0);
            ...
            Validate();Capture();
            File.WriteAllText(Reports+"/summary.json",JsonUtility.ToJson(new Summary{treeInstances=data.trees.Length,sharedTreeMeshes=15,staticChunks=data.meshes.Count(m=>m.kind=="opaque"||m.kind=="water"),trianglesInUniqueMeshes=data.meshes.Sum(m=>m.triangles.Length/3),scene=ScenePath},true));
        }
        [Serializable] class Summary {public int treeInstances,sharedTreeMeshes,staticChunks,trianglesInUniqueMeshes;public string scene;}
        ...
        public static void Capture()
        {
            ...
            try{Shot(cam,new Vector3(0,3,0),new Vector3(52,-38,0),113,"overview");Shot(cam,new Vector3(-27,3,-8),new Vector3(55,-38,0),23,"patrol");Shot(cam,new Vector3(46,4,32),new Vector3(65,-20,0),26,"base");}
        ...
        static void PlayState(PlayModeStateChange state)
        {
            ...
            EditorApplication.delayCall+=()=>
            {
                try
                {
                    ...
                    var points=Object.FindObjectsByType<ForestPoint>(FindObjectsSortMode.None);m.Interact(points.Single(p=>p.id=="supplies"));if(m.Stage!=1)throw new InvalidOperationException("Supply interaction failed");
                    File.WriteAllText(Reports+"/playcheck-result.txt","PASS: mission initialized; three guards active; supplies interaction advances stage; content references resolved.");
                }
                catch(Exception e){File.WriteAllText(Reports+"/playcheck-result.txt","FAIL: "+e);}
                finally{SessionState.SetBool("SV.Map01.PlayCheck",false);EditorApplication.isPlaying=false;}
            };
        }
    }
}
