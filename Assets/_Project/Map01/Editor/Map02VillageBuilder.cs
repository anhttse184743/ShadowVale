...
namespace ShadowVale.Map01.Editor
{
    [InitializeOnLoad] public static class Map02VillageBuilder
    {
        ...
        static void Import()
        {
            OptimizedMapBuilder.SourceData data;using(var f=File.OpenRead("SourceArt/Map02_Village/Village.meshdata.json.gz"))using(var z=new GZipStream(f,CompressionMode.Decompress))using(var r=new StreamReader(z))data=JsonUtility.FromJson<OptimizedMapBuilder.SourceData>(r.ReadToEnd());
        ...
        {
            int guards=Object.FindObjectsByType<ForestGuard>(FindObjectsSortMode.None).Count(g=>g.gameObject.scene==env.gameObject.scene);if(guards!=0)throw new Exception("Enemy objects present");
            var checks=new List<string>{"PASS continuous terrain 220 x 200 = 44000 m2; same footprint as Map 1","PASS straight river centre x="+RiverX+" at one-third width; 10.5m = 1.5 x Map 1 nominal 7m","PASS no enemy models / ForestGuard components in Map 2","PASS 14 houses, 18 rice paddies, two evacuation landings, separate design-only story markers"};
