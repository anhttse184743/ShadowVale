    ...
    public sealed partial class ForestMission
    {
        private void Aim()
        {
            if (modernPlayer != null) {
                var modernRay = gameCamera.ViewportPointToRay(new Vector3(.5f, .5f));
                aim = Physics.Raycast(modernRay, out var hit, Weapon.range, ObstructionMask | LayerMask.GetMask("Enemy"), QueryTriggerInteraction.Ignore)
                    ? hit.point : modernRay.GetPoint(Weapon.range);
                return;
            }
