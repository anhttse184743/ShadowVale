...
{
    /// <summary>Map 1's painted-metal HUD: health bar, objective plate, inventory, quick-use bar,
    /// drag-and-drop, dialogue feedback and the map overlay. Pure presentation — reads
    /// Map01Mission/Map01Quest/Map01Inventory/Map01PlayerInteraction, owns nothing gameplay.</summary>
    public sealed class Map01Hud : MonoBehaviour
    {
        ...
        private void DrawHud()
        {
            ...
            DrawHealth();
            DrawObjective();
            if (mission.ModernCombat != null && !mission.InventoryOpen && !mission.MapOpen) {
                var weapon = mission.ModernCombat.EquippedKind;
        ...
        private void DrawHealth()
        {
            ...
            var fill = new Rect(bar.x, bar.y, bar.width * Mathf.Clamp01(mission.PlayerHealth / mission.Settings.playerHP), bar.height);
        ...
        private void DrawObjective()
        {
            ...
            GUI.Label(new Rect(49, 258, 328, 69), Map01Quest.Objectives[Mathf.Clamp(quest.Stage, 0, Map01Quest.CompleteStage)], hudBody);
        }
        private void DrawInventory(float width)
        {
            ...
            if (selectedItem != null) {
                ...
                GUI.Label(new Rect(detail.x + 153, detail.y + 122, 490, 29), $"Tổng: {inventory.Count(selectedItem)}  ·  Tối đa {inventory.StackLimit(selectedItem)}/ô", hudSmall);
        ...
        private void DrawHudFeedback(float width, float height)
        {
            ...
            if (quest.HungInRange && !mission.Stopped && !mission.InventoryOpen)
                GUI.Label(new Rect(x + 18, height - 338, areaWidth - 36, 43),
                    inventory.Count("herb") > 0 ? "[E] Dùng thảo dược chữa trị cho Hùng" : "Cần thảo dược để chữa trị cho Hùng", hudBody);
            ...
            if (inventory.IsCrafting) GUI.Label(new Rect(47, 350, 415, 40), $"Đang chế tạo… {inventory.CraftRemaining:0.0}s", hudBody);
            foreach (var enemy in mission.Enemies.Where(e => e.Alive && e.Alerted)) {
                var screen = mission.gameCamera.WorldToScreenPoint(enemy.transform.position + Vector3.up * 2.4f);
                var point = new Vector2(screen.x / HudScale, (Screen.height - screen.y) / HudScale);
                if (screen.z > 0 && (!mission.InventoryOpen || !InventoryRect(width).Contains(point)) && Vector3.Distance(mission.player.position, enemy.transform.position) < 24)
                    GUI.Label(new Rect(point.x - 60, point.y, 190, 40), "CẢNH GIÁC", hudSmall);
            }
        ...
        private void DrawMap(float width, float height)
        {
            ...
            GUI.Label(new Rect(rect.x + 20, rect.y + 10, 580, 35), "TUYẾN VẬN CHUYỂN / M để đóng", hudHeading);
            GUI.Label(new Rect(rect.x + 20, rect.y + 60, 260, 300), "BẮC ↑\n\n05  Bến sông / rút lui\n04  Căn cứ / bàn tài liệu\n03  Điểm nghỉ / chế tạo\n02  Khu tuần tra\n      Tây: bụi rậm\n      Giữa: vật chắn\n      Đông: đánh lạc hướng\n01  Nhận hàng / xuất phát", hudBody);
            var area = new Rect(rect.x + 310, rect.y + 60, 280, 335);
            foreach (var p in mission.Points.Where(p => p.kind != ForestPointKind.Cover && p.kind != ForestPointKind.Hide))
            {
            ...
            GUI.color = Color.cyan; var playerPos = MapPosition(mission.player.position, area);
