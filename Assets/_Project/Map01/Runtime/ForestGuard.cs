...
namespace ShadowVale.Map01
{
    ...
    public sealed class ForestGuard : MonoBehaviour
    {
        ...
        [System.Serializable] public sealed class Snapshot
        {
            ...
            public int waypoint;
            public bool stopped;
            public int combatVersion, burstLeft, rounds, shotsFired;
            public float orderRemaining;
        ...
        public Snapshot Capture() => new Snapshot {
            ...
            combatVersion = 1, burstLeft = burstLeft, rounds = rounds, shotsFired = ShotsFired,
            orderRemaining = Mathf.Max(0, orderUntil - Time.time), orderPosition = orderPosition
        };
        public void RestoreSnapshot(Snapshot data)
        {
            ...
            if (data.combatVersion > 0) {
                burstLeft = data.burstLeft; rounds = data.rounds; ShotsFired = data.shotsFired;
                orderUntil = Time.time + data.orderRemaining; orderPosition = data.orderPosition;
            }
            nextPath = 0;
            Go(data.destination);
