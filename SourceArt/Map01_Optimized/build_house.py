...
ROOT=r'G:\game\ShadowVale';OUT=os.path.abspath('outputs/Map01_Polish');os.makedirs(OUT,exist_ok=True)
...
def house(lod):
 ...
 for x in [-2.9,0,2.9]:
  for y in [-4,2.35]:g.rod([(x,y,-.2),(x,y,2.7)],[.105,.085],6,6)
 ...
 for side,extent in [(-1,4.5),(1,2.9)]:
  ...
  for row in range(rows):
   ...
   for j in range(columns):
    x=-3.6+j*7.2/columns;w=7.2/columns*1.12;tip=t1+random.uniform(-.012,.012)
    def z(t):return 4.6-1.85*t-.16*math.sin(t*math.pi)
 ...
 g.rod([(-3.65,0,4.64),(3.65,0,4.64)],[.13,.13],7,6)
