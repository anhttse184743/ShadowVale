Shader "ShadowVale/Natural Surface"
{
        ...
        Pass
        {
            ...
            half4 Frag(V i, FRONT_FACE_TYPE front:FRONT_FACE_SEMANTIC):SV_Target
            {
                ...
                half3 light=SampleSH(n)+l.color*(.16h+.84h*ndl)*l.shadowAttenuation;
                                half isLeaf=step(i.c.r*1.12h,i.c.g);
                half bark=Noise(i.world.xz*35+i.world.y*.25);
                half grain=lerp(.72h+.40h*bark,.88h+.20h*Noise(i.world.xz*18+i.world.y*3),isLeaf);
                half backLight=pow(saturate(dot(-n,l.direction)),2)*.15h*isLeaf;
                light+=l.color*backLight;
